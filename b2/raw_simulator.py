...
class BucketSimulator(object):
    ...
    def __init__(
        ...
        self.revision = 1
        self.upload_url_counter = iter(list(range(200)))
        # File IDs count down, so that the most recent will come first when they are sorted.
        self.file_id_counter = iter(list(range(self.FIRST_FILE_NUMBER, 0, -1)))
        self.upload_timestamp_counter = iter(list(range(5000, 9999)))
...
class RawSimulator(AbstractRawApi):
    ...
    def __init__(self):
        ...
        self.bucket_id_counter = iter(list(range(100)))
