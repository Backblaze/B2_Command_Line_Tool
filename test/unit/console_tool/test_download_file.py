...
def test_download_file_by_name(b2_cli, bucket, test_file_setup):
    ...
    b2_cli.run(
        ['download-file-by-name', '--noProgress', 'my-bucket', 'file1.txt', str(local_file)],
...
def test_download_file_by_name_quietly(b2_cli, bucket, test_file_setup):
    ...
    b2_cli.run(
        ['download-file-by-name', '--quiet', 'my-bucket', 'file1.txt', str(local_file)],
        expected_stdout=''
    )
    assert local_file.read() == content
...
def test_download_file_by_id(b2_cli, bucket, test_file_setup):
    ...
    b2_cli.run(
        ['download-file-by-id', '--noProgress', '9999', str(local_file)],  # <-- Here's the change
        expected_stdout=EXPECTED_STDOUT_DOWNLOAD
    )
    assert local_file.read() == content
...
def test_download_file_by_id_quietly(b2_cli, bucket, test_file_setup):
    ...
    b2_cli.run(
        ['download-file-by-id', '--quiet', '9999', str(local_file)],  # <-- Here's the change
        expected_stdout=''
    )
    assert local_file.read() == content
